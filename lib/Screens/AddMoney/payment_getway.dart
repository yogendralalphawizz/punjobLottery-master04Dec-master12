...
import '../../Models/payment_check_model.dart';
...
class _PaymentgetwayState extends State<Paymentgetway> {
  ...
  PaymentStatusModel paymentStatusModel=PaymentStatusModel();

  paymentCheck() async {
    Map<String,String> body={

      'amount':widget.amount,
      'OrderKeyId':widget.orderid,

    };
    print("${body}"+"++++++++++++++");
    final response = await
    http.post(Uri.parse('${baseUrl}payment_check'), body:body );

    var data=  jsonDecode(response.body);
    if (response.statusCode == 200) {


      paymentStatusModel=PaymentStatusModel.fromJson(data);
      setState(() {
